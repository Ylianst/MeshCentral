    ...
    InstallModules(['minimist'], {}, function () {
        ...
        for (var i in config.domains) {
            ...
            if ((typeof config.domains[i].authstrategies == 'object')) {
                ...
                if ((typeof config.domains[i].authstrategies.oidc == 'object') && ((typeof config.domains[i].authstrategies.oidc.clientid == 'string') || (typeof config.domains[i].authstrategies.oidc.client?.client_id == 'string')) && ((typeof config.domains[i].authstrategies.oidc.clientsecret == 'string') || (typeof config.domains[i].authstrategies.oidc.client?.client_secret == 'string')) && ((typeof config.domains[i].authstrategies.oidc.issuer == 'string') || (typeof config.domains[i].authstrategies.oidc.issuer?.issuer == 'string')) && (passport.indexOf('openid-client') == -1)) {
                    if ((nodeVersion >= 17) || ((Math.floor(nodeVersion) == 16) && (nodeVersion >= 16.13)) || ((Math.floor(nodeVersion) == 14) && (nodeVersion >= 14.15)) || ((Math.floor(nodeVersion) == 12) && (nodeVersion >= 12.19))) { passport.push('openid-client'); } else { addServerWarning('This NodeJS version does not support OpenID.', 25); delete config.domains[i].authstrategies.oidc; }
                }
