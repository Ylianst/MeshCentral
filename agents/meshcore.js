...
{
    // Consent is scoped to the session: never let a granted microphone survive
    // the tunnel that asked for it.
...
// Writable stand-in placed between the browser tunnel and the native KVM
// stream. Its only job is to hold MNG_MIC_START until the local user has
// agreed, then forward everything unchanged.
//
// The interception cannot live on the KVM stream itself: that object is
// created in native code and exposes write as a readonly property, so
// assigning a wrapper over it silently does nothing. It cannot live in
// onTunnelData either, because a tunnel with remote control rights is piped
// directly into the KVM stream and never passes through that function.
function micCreateConsentRelay(tunnel) {
    return {
        write: function (chunk) {
            try {
                ...
                // MNG_MIC_CONSENT (100) is the agent telling its own native
                // layer that the user agreed. A browser must never be able to
                // send it, so drop it if it arrives from the tunnel.
                if (cmd == 100) { return true; }
                if (cmd == 97) { // MNG_MIC_START
                    if (micConsentHandleStart(tunnel) === false) { return true; }
                }
            } catch (ex) { }
            try { tunnel.httprequest.desktop.kvm.write(chunk); } catch (ex) { }
            return true;
        },
        end: function () { }
    };
...
function micConsentHandleStart(tunnel) {
    ...
    httprequest.micConsentPending = true;

    var consentMessage = currentTranslation['micConsent'].replace(/\{0\}/g, httprequest.realname).replace(/\{1\}/g, httprequest.username);
    var consentTitle = 'MeshCentral';
    if (httprequest.soptions != null) {
        if (httprequest.soptions.consentTitle != null) { consentTitle = httprequest.soptions.consentTitle; }
        if (httprequest.soptions.consentMsgMic != null) { consentMessage = httprequest.soptions.consentMsgMic.replace(/\{0\}/g, httprequest.realname).replace(/\{1\}/g, httprequest.username); }
    }

    tunnel.write(JSON.stringify({ ctrlChannel: '102938', type: 'console', msg: "Waiting for user to grant microphone access...", msgid: 1 }));

    var pr;
    if (process.platform == 'win32') {
        // Prefer the same rich dialog used elsewhere, falling back to the
        // basic message box where it is unavailable.
        var enhanced = false;
        ...
        if (enhanced) {
            ...
            // Never auto-accept a microphone on timeout: silence is not consent.
...
function micConsentGranted() {
    ...
    // Write straight to the KVM stream, bypassing the relay that blocks this
    // command when it comes from a browser.
    try { httprequest.desktop.kvm.write(Buffer.from(String.fromCharCode(0x00, 100, 0x00, 0x04))); } catch (ex) { }
    ...
    if (httprequest.consent && (httprequest.consent & 1)) {
        ...
        if (httprequest.soptions != null) {
            ...
            if (httprequest.soptions.notifyMsgMic != null) { notifyMessage = httprequest.soptions.notifyMsgMic.replace(/\{0\}/g, httprequest.realname); }
        }
        try { require('notifybar-desktop')(notifyTitle + '\n' + notifyMessage, require('MeshAgent')._tsid); } catch (ex) { }
    }
}

function micConsentDenied(e) {
    var tunnel = this.tunnel;
    if ((tunnel == null) || (tunnel.httprequest == null)) { return; }
    var httprequest = tunnel.httprequest;
    ...
    // Report the refusal instead of failing silently, so the operator is not
    // left wondering whether they are being heard. The desktop session itself
        ...
        if (this.httprequest.protocol == 0) { // 1 = Terminal (admin), 2 = Desktop, 4 = Registry, 5 = Files, 6 = PowerShell (admin), 7 = Plugin Data Exchange, 8 = Terminal (user), 9 = PowerShell (user), 10 = FileTransfer
            ...
            else if (this.httprequest.protocol == 2)
            {
                ...
                if ((this.httprequest.desktopviewonly != true) && ((this.httprequest.rights == 0xFFFFFFFF) || (((this.httprequest.rights & MESHRIGHT_REMOTECONTROL) != 0) && ((this.httprequest.rights & MESHRIGHT_REMOTEVIEW) == 0))))
                {
                    ...
                    // Pipe through a relay rather than straight into the KVM
                    // stream, so microphone frames can be inspected on the way
                    // past. The KVM stream is native and exposes write as a
                    // readonly property, so it cannot be wrapped in place;
                    // assigning over it fails silently.
                    this.pipe(micCreateConsentRelay(this), { dataTypeSkip: 1, end: false }); // 0 = Binary, 1 = Text. Pipe the Browser --> KVM input.
                }
                else
                {
