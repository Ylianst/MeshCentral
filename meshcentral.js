        ...
        if (obj.platform == 'linux') {
            if (obj.args.install == true) {
                ...
                require('child_process').exec('which node', {}, function (error, stdout, stderr) {
                    ...
                    // NODE_OPTIONS=--experimental-require-module is required on Node 22 (<22.12) to allow CommonJS require() of ES Modules. otplib v13 pulls @scure/base (ESM-only) via @otplib/plugin-base32-scure, so 2FA TOTP setup fails silently on Node 22 without this flag. From Node 22.12+ this is the default behaviour and the flag becomes a harmless deprecation warning.
